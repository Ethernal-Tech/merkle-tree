...
func newMerkleNode(left, right *MerkleNode, data []byte, hasher hash.Hash) *MerkleNode {
	...
	node.hash = BytesToHash(hasher.Sum(nil))
...
func NewMerkleTree(data [][]byte) (*MerkleTree, error) {
	return NewMerkleTreeWithHashing(data, NewKeccakState())
}
...
func VerifyProof(index uint64, leaf []byte, proof []Hash, root Hash) error {
	return VerifyProofUsing(index, leaf, proof, root, NewKeccakState())
}
...
func getProofHash(index uint64, leaf []byte, proof []Hash, hasher hash.Hash) []byte {
	hasher.Write(leaf)
